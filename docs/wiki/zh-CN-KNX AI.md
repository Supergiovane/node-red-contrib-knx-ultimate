...
### ETS 对象访问
此部分复用 IoT Bridge MQTT 配置中的组地址选择器。可以筛选导入列表、全选或全不选，并对当前显示的地址批量或逐行设置只读。只有选中的地址可供模型使用。只读地址仍然可见并允许 `GroupValue_Read`，但本地验证会拒绝所有 `GroupValue_Write`，即使其角色为 command。不存在迁移或旧版回退：升级后必须打开每个现有 KNX AI 节点，保存明确选择并部署；在此之前，其 AI 目录为空。
