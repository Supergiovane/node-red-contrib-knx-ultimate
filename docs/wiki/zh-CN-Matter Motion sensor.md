...
lang: zh-CN
permalink: /wiki/zh-CN-Matter%20Motion%20sensor
---
# Matter 运动传感器 (BETA)

> 此节点为 **BETA**：Matter 实现继续完善时，行为可能会变化。

此节点把 运动端点 连接到 KNX，也可以连接到 Node-RED 输出。
...
| Matter 运动传感器 | 从已配网设备中选择的 运动端点。列表会过滤为暴露 `OccupancySensing` 的端点。 |
| 运动组地址 | 运动组地址，接收转换后的值。默认 DPT：`1.001`。 |
| Read at startup | 在部署/启动或设备重新连接时发布缓存的 Matter 值。 |
| Node output | 显示 Node-RED 输出端口，并发送每一次 Matter 更新。 |

## 行为

节点读取 `OccupancySensing.occupancy`，转换为布尔占用/运动状态，写入配置的 KNX 组地址，并用最后已知值响应 KNX 读取请求。
