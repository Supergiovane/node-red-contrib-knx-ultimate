...
### Modifica e backup dell'apprendimento CHAT e della memoria Cerebrum
La scheda **Cerebrum (BETA)** nella configurazione Node-RED di KNX AI include pulsanti diretti per **Apprendimento AI Chat** e **Memoria Cerebrum** nella Web UI Vue.

Nella UI web Vue, apri **Cerebrum → Apprendimento AI Chat** per visualizzare il file condiviso esatto `knxai-chat-context.knxctx` e il suo percorso assoluto. **File nativo** espone i record autorevoli modificabili, mentre **Testo semplificato** presenta le stesse conversazioni, istruzioni apprese e sorveglianze telecamera come spiegazione localizzata di sola lettura. La copia segue la vista selezionata; download e ripristino usano sempre il backup nativo completo. **Re-inizializza memoria**, protetto da una conferma esplicita, sostituisce il file con un nuovo contesto vuoto e cancella sessioni, istruzioni, sorveglianze telecamera e conferme chat pendenti in ogni nodo KNX AI che usa lo stesso archivio. I record nativi separati da tabulazioni `KNXAI_CHAT_CONTEXT 3` usano `SESSION`, `INSTRUCTION`, `TURN`, `CAMERA_WATCH` ed `END_SESSION`. Il salvataggio valida e limita questi record, riscrive atomicamente il file e aggiorna il contesto attivo di ogni nodo KNX AI che usa lo stesso archivio. Un controllo di revisione impedisce di sovrascrivere o azzerare l'apprendimento cambiato dopo il caricamento nell'editor.
...
Apri **Cerebrum → Memoria Cerebrum** per esaminare e modificare il file condiviso `knxai-home-memory.md`. La vista **JSON** contiene i dati autorevoli modificabili, mentre **Testo semplificato** presenta le stesse abitudini, decisioni degli occupanti, stati, osservazioni, notifiche e oggetti conosciuti come spiegazione localizzata di sola lettura. Il browser ricorda la vista scelta. Il salvataggio valida il JSON e rigenera le sezioni leggibili del file; la copia segue la vista corrente, mentre download e ripristino restano sempre backup completi. Il controllo di revisione protegge dalle modifiche concorrenti.

La pagina Web **Impostazioni** contiene soltanto **Import / Export**. Il backup rigoroso versione 1 comprende la configurazione persistente KNX AI, Apprendimento AI Chat, memoria della casa ed entrambi i file delle pianificazioni. La pagina usa sempre il nodo KNX AI distribuito che l’ha aperta, non mostra un secondo selettore e non accetta i vecchi formati di esportazione.
...
Da gerarchia ETS, nomi e DPT, il nodo crea un modello semantico deterministico per persiane, finestre, porte, luci, temperatura, clima, presenza e allarmi usando termini italiani, inglesi, tedeschi, francesi, spagnoli e cinesi. Il rilevatore proattivo osserva soltanto stati in sola lettura di persiane, finestre e porte riconosciuti con sufficiente affidabilità.
...
Cerebrum apprende anche pattern temporali limitati fra giorni feriali e fine settimana, ricavati dalle scritture KNX e dai cambi di stato HUE, Matter o Home Assistant. Prima di usare una sola volta l'LLM per formulare una proposta attende almeno otto osservazioni coerenti, distribuite su sei date distinte e su un intervallo minimo di 14 giorni, con confidenza non inferiore a 0,70. Telegrammi ripetuti nella stessa giornata non possono quindi anticipare la richiesta di conferma. La proposta viene inviata all'occupante ricordato sull'uscita 3/Telegram. L'occupante può confermare, rifiutare oppure rispondere in linguaggio naturale con una correzione, per esempio un altro orario o giorno. Decisione e testo originale vengono salvati. Solo un'abitudine confermata può in seguito generare un suggerimento anticipato fino a 30 minuti prima dell'orario abituale. Proposte e suggerimenti non autorizzano mai un'esecuzione: le azioni KNX e Home Assistant seguono il normale percorso di autorizzazione e conferma, e resta valido il limite globale di tre messaggi proattivi all'ora.
...
La raccolta degli stati parte dagli eventi e non interroga l'LLM. Un riconciliatore ogni 15 secondi gestisce soltanto dati scaduti o mancanti: gli snapshot Home Assistant usano intervalli adattivi caldo/tiepido/freddo (2/10/30 minuti), mentre, quando l'accesso agli stati KNX è abilitato, gli aggiornamenti KNX sono `GroupValue_Read` deterministici sui soli oggetti semantici selezionati in sola lettura, al massimo uno per tick e 60 all'ora. Ogni stato osservato riceve un livello hot/warm/cold in base all'attività recente e resta locale. Nei prompt entra soltanto una porzione della cache ordinata per rilevanza e limitata in dimensione, quindi il contesto scala con la domanda e non con tutta la casa.

L'ultima sessione chat viene ricordata come proprietario e riceve i messaggi spontanei. L'uscita 3 emette notifiche localizzate e proposte di abitudini; un `msg.inputMessage` sintetico conserva la sessione per l'adattatore chat. Un limite rigido di tre notifiche proattive all'ora evita abusi. Il riconciliatore può emettere sull'uscita 4 soltanto letture KNX di aggiornamento, ma non emette mai una scrittura autonoma né modifica attuatori; un'eventuale richiesta successiva passa sempre dalla normale validazione e conferma.

Il riferimento appreso condiviso viene caricato all'avvio da `<userDir>/knxai/memory/knxai-home-memory.md`, aggiornato dopo i cambiamenti significativi, riscritto atomicamente ogni 15 minuti e sempre limitato rigidamente a 5 MB. Conserva al massimo 120 osservazioni significative, 80 abitudini aggregate, 120 decisioni degli occupanti, 80 notifiche, 300 oggetti ETS semantici e 600 stati correnti, mai un flusso illimitato di telegrammi raw. Gli elementi vecchi e meno importanti vengono eliminati per primi.
