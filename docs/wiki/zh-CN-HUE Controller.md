...
- 在保存编辑器之前，选择其他 Hue 资源仅作为预览。按下`取消`会恢复之前保存的设备、功能和映射。
- 只有在选择 Hue Bridge、成功连接并加载其资源目录后，Hue 设备选择器才会显示。等待期间会显示加载指示器。将 bridge 重新设为 `none` 时，加载指示器、设备选择器和设备编辑器会再次隐藏。
- 已配置的节点会在所选 bridge 离线时继续显示已保存的 Hue 设备选择器，但该选择器处于禁用状态；KNX 映射选项卡仍然可用。15 秒后，等待标签会提示无法访问 Hue Bridge。资源发现会在后台继续；如果 bridge 始终未能就绪，最终会显示固定的红色错误消息。
- 将 KNX 网关设为 `none` 时只会隐藏灯光映射选项卡。已挂载的 GA、DPT 和名称字段保持不变，并在重新选择网关后与正确的垂直选项卡布局一起立即显示。
- Node-RED 在初始化 KNX 和 Hue 配置选择器时以编程方式产生的临时占位值不会覆盖已保存的网关。只有用户明确选择 `none` 时才会隐藏灯光映射选项卡。
- 灯光映射选项卡现在只包含配置控件；调光、可调白光和 RGB 装饰图片已移除。
- 灯具映射会根据能力显示：调光、可调白光、RGB/HSV 和原生效果遵循所选 Hue API v2 灯资源实时提供的 `dimming`、`color_temperature`、`color` 和 `effects` 属性。
- 非阻塞灯光编辑器：已保存的映射会立即显示，无需等待运行时 Hue 资源缓存。当前能力在后台加载；失败时映射和引脚选择器仍然可用，并显示固定的红色 Node-RED 错误消息。
- 弹性灯光编辑器：Locate 和映射容器会先于可选的效果与选项卡控件初始化。编辑器启动时暂时为空的选择值不会覆盖已保存的 KNX 网关。浏览器端错误或首次 Locate 命令被拒绝时，Node-RED 会显示固定的红色错误消息和技术详情，而不会让编辑器无提示地保持沉默。
...
- 可完全在本地迁移全部十五种旧版节点类型；随后打开可编辑的邮件草稿，在最终消息中提供可选支持按钮，并在本地检查后手动 Deploy。
...
仅当 Node-RED 编辑器在当前流程中检测到至少一个旧版 HUE 节点时，迁移按钮才会显示。HUE Controller 和每个旧版 HUE 编辑器中，带白色文字的高对比度橙色按钮前都会显示一个[迁移说明视频的 YouTube 链接](https://youtu.be/f0Evf2QFI7c)。按钮下方的声明确认不会有任何流程或节点数据离开浏览器。

点击**转换旧版 HUE 节点**并确认。浏览器会在本地完成全部转换，不会发送任何流程、节点、`hue-config`、`knxUltimate-config`、组地址、连线、凭据、名称、位置或 ID 数据。转换成功后，它只会打开一封发给作者的可编辑邮件草稿，而不会离开 Node-RED。草稿只包含已转换节点数量和可选备注空间；是否发送由你决定，绝不会自动发送。最终的 Node-RED 消息会提供一个可选支持按钮；只有点击该按钮时才会打开捐赠页面。
...
浏览器会关闭当前节点编辑器，并只把检测到的旧版 HUE 节点转换为 HUE Controller 实例。已保存的属性、配置引用、位置、分组和连线保持不变。工作区会标记为已修改，但工具绝不会自动部署：请检查结果并自行点击 **Deploy**。节点已变化、流程被锁定或发生本地转换错误时，工作区不会发生变化。**安全检查：**点击 Deploy 前，请逐一检查所有已修改 HUE 节点的功能、配置引用、输入/输出端口和连线。处理完成后，Node-RED 的固定消息会一直显示，直到点击**确定**。
...
未选择 KNX 网关时，将以仅通过流程输入使用的提示替换重复的“Philips HUE”标题。

**灯组：**配置 KNX 网关后，选择 `grouped_light` 将始终显示完整的开关、调光、可调白光、RGB/HSV、效果和行为映射。编辑器不会根据灯组当前包含的灯具限制这些字段。
