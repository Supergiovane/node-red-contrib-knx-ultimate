...
- 紧凑的 KNX 映射行会将 GA、DPT 和名称保持在同一行；DPT 与名称控件采用较小宽度，在较窄的编辑器中名称字段还可继续收缩，且不会改变已保存的值。
...
- 可在本地通过复制/粘贴迁移全部十五种旧版 HUE 节点类型，不读取或修改当前流程。
...
仅当 Node-RED 编辑器在当前流程中检测到至少一个旧版 HUE 节点时，迁移按钮才会显示。每个旧版 HUE 编辑器的弃用提示下方也提供相同的橙色按钮。这只是只读的可见性检测：转换器仍只处理您明确粘贴的 JSON。

点击**转换旧版 HUE 流程**，粘贴完整的 Node-RED 流程导出内容，然后点击**转换**。该工具完全在浏览器中运行，不会读取或修改当前打开的流程。输出会将所有受支持的旧版 HUE 节点替换为 HUE Controller，同时保留 ID、配置引用、属性、位置和连线。复制生成的 JSON，建议先作为新流程导入并进行比较，再删除原流程。

Node-RED 会特意从流程导出中排除凭据。重新导入同一个实例时，请保持选中**复用现有 Hue 和 KNX 配置节点**。转换器会省略被引用的 `hue-config` 和 `knxUltimate-config` 对象，但在每个转换后的节点中保留其 ID，因此 Node-RED 会继续使用包含现有密钥和密码的配置。仅在目标为另一个实例时取消选中；此时需要在导入后重新输入凭据。

**复制转换后的流程**会先使用浏览器 Clipboard API，再使用真正被选中的临时文本框作为备用方式。如果浏览器阻止两种方法，输出 JSON 会被选中，以便手动复制。
