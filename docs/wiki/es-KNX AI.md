...
Para los proveedores locales, **Cantidad de contexto del chat** permite elegir explícitamente 4K, 8K, 16K o sin límite de KNX AI; 16K sigue siendo el valor predeterminado. Sin límite de KNX AI se usa la ventana de contexto declarada o activa del modelo, sin superar nunca su máximo físico. La selección limita proporcionalmente los datos KNX, de memoria, del proyecto Node-RED y de los adaptadores enviados al modelo, manteniendo completo el contrato de herramientas del agente. Ninguna capacidad se activa o desactiva según frases, palabras clave o intents lingüísticos.
...
- El contexto máximo declarado por `/api/show` queda solo como información. KNX AI envía como `num_ctx` el presupuesto elegido de 4K, 8K o 16K; sin límite de KNX AI usa el contexto declarado por el modelo, sin superar nunca su máximo físico. Cada fuente de contexto se limita proporcionalmente sin eliminar capacidades del agente.
...
- Si un modelo ya está cargado, KNX AI conserva la longitud de contexto activa. KNX AI nunca carga un modelo Bionic inactivo mediante la API de gestión: la primera solicitud de chat permite que Bionic lo cargue mediante JIT con los valores predeterminados guardados para el modelo. KNX AI usa el presupuesto de prompt elegido de 4K, 8K o 16K, o la ventana declarada o activa del modelo cuando se selecciona sin límite de KNX AI, sin superar su máximo físico; el razonamiento, KNX, las rutinas, las cámaras y TTS permanecen disponibles.
