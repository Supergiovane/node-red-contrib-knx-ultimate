...
À partir de la hiérarchie ETS, des noms et DPT, le nœud crée un modèle sémantique déterministe. Le détecteur proactif ne considère que les états en lecture seule reconnus de façon fiable. Il n’existe ni interrupteur séparé ni paramètres proactifs avancés. Une notification n’est évaluée que si le LLM est actif et si **Éducation IA** la demande explicitement. L’Éducation définit seule les conditions, la durée, les heures silencieuses et la répétition. Sans règle explicite, ou si le LLM ne peut pas l’évaluer, aucun message n’est envoyé.
