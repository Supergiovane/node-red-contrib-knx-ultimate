...
Aus ETS-Hierarchie, Namen und DPTs erstellt der Node ein deterministisches semantisches Modell. Der proaktive Detektor berücksichtigt nur zuverlässig erkannte Nur-Lesen-Zustände. Es gibt keinen separaten Schalter und keine erweiterten proaktiven Einstellungen. Eine Benachrichtigung wird nur bewertet, wenn das LLM aktiv ist und die **KI-Erziehung** sie ausdrücklich verlangt. Ausschließlich die Erziehung bestimmt Bedingungen, Offenzeit, Ruhezeiten und Wiederholung. Ohne eine ausdrückliche Regel oder bei fehlgeschlagener LLM-Auswertung wird nichts gesendet.
