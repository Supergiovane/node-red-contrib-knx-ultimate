...
### Editing and backing up CHAT learning and Cerebrum memory
The **Cerebrum (BETA)** tab in the Node-RED KNX AI configuration includes direct buttons for both **AI Chat Learning** and **Cerebrum Memory** in the Vue Web UI.

In the Vue web UI, open **Cerebrum → AI Chat Learning** to view the exact shared `knxai-chat-context.knxctx` file and its absolute path. **Native file** exposes the authoritative editable records, while **Simplified text** presents the same conversations, learned instructions and camera watches as a localized read-only explanation. Copy follows the selected view; download and restore always use the complete native backup. **Reinitialize Memory**, protected by an explicit confirmation, replaces it with a new empty context and clears saved sessions, instructions, camera watches and pending chat confirmations across every KNX AI node using the same storage. The native tab-separated `KNXAI_CHAT_CONTEXT 3` records use `SESSION`, `INSTRUCTION`, `TURN`, `CAMERA_WATCH` and `END_SESSION`. Saving validates and bounds these records, atomically rewrites the file and updates the live context of every KNX AI node using the same storage. A revision check refuses to overwrite or reset learning that changed after the editor loaded it.
...
Open **Cerebrum → Cerebrum Memory** to inspect and edit the shared `knxai-home-memory.md`. The **JSON** view contains the authoritative editable data, while **Simplified text** renders the same habits, occupant decisions, states, observations, notifications and known home objects as a localized read-only explanation. The selected view is remembered by the browser. Saving validates the JSON and regenerates the readable file sections; copy follows the current view, while download/restore always remains a complete backup. Revision checks protect concurrent experience.

The Web UI **Settings** page contains only **Import / Export**. Its strict version-1 backup includes the persisted KNX AI configuration, AI Chat Learning, home memory and both schedule files. The page always uses the deployed KNX AI node that opened it and does not expose a second selector or accept older export formats.
...
Cerebrum also learns bounded weekday/weekend time patterns from KNX writes and HUE, Matter or Home Assistant state changes. It waits for at least eight consistent observations spread across six distinct dates and a minimum 14-day span, with confidence of at least 0.70, before using the LLM once to phrase a proposal and sending it to the remembered occupant on output 3/Telegram. Repeated telegrams on the same day therefore cannot accelerate confirmation. The occupant can confirm, reject or answer naturally with a correction such as another time or day. The decision and original wording are persisted. Only a confirmed habit can later produce an anticipatory suggestion up to 30 minutes before its usual time. A proposal or suggestion is never execution authority: KNX and Home Assistant actions still use the normal authorization and confirmation path, and the global limit of three proactive messages per hour still applies.
...
State collection is event-first and does not ask the LLM. A 15-second reconciler only handles stale or missing data: Home Assistant snapshots use adaptive hot/warm/cold intervals (2/10/30 minutes), while, when KNX state access is enabled, KNX refreshes are deterministic `GroupValue_Read` operations for selected read-only semantic objects, at most one per tick and 60 per hour. Every observed state is assigned a hot/warm/cold tier from recent change activity and stored locally. Conversation prompts receive only a relevance-ranked, size-bounded slice of this cache, so context grows with the question instead of with the whole house.

The most recent chat session is remembered as the owner and receives spontaneous messages. Output 3 emits localized proactive messages and habit proposals; a synthetic `msg.inputMessage` preserves the session for the chat adapter. A hard safety limit of three proactive messages per hour prevents flooding. The reconciler may emit read-only KNX refreshes on output 4, but it never emits an autonomous write or changes an actuator; a subsequent user request still uses the normal validation and confirmation workflow.

The shared learned reference is loaded at startup from `<userDir>/knxai/memory/knxai-home-memory.md`, updated after meaningful changes, rewritten atomically every 15 minutes and always hard-capped at 5 MB. It stores at most 120 significant observations, 80 aggregate habits, 120 occupant decisions, 80 notifications, 300 semantic ETS objects and 600 current states—never a raw unlimited telegram stream. Older low-priority entries are removed first.
