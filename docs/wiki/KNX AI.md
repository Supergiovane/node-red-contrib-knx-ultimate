...
## LLM context (better answers)
When you use `ask`, the node can optionally send extra context to the LLM:
- **Flow inventory**: a list of KNX Ultimate nodes found in your Node-RED flows (helps mapping telegrams to the logic).
- **Documentation snippets**: relevant excerpts from built-in help/README/examples (and `docs/wiki` when available).
...
## Web dashboard
- Click **Open Web Page** in the KNX AI node editor to open the full KNX AI dashboard.
- Direct URL: `/knxUltimateAI/sidebar/page` (optionally add `?nodeId=<id>`).
- The previous KNX AI sidebar plugin has been replaced by this web page.
- The dashboard includes Flow Map, Event Mix, anomalies list, and Ask chat.
