        ...
        node.on("input", function (msg) {
            ...
            if (typeof msg.payload === "undefined" || typeof msg.payload === "object" || typeof msg.payload === "function") // 16/01/2020 Check for invalid payload
            {
                ...
                if (!msg.hasOwnProperty("readstatus") && !msg.payload.hasOwnProperty("decr_incr")) {
                    ...
                    RED.log.error("knxUltimateWatchDog: Node " + node.id + " has received an INVALID payload. Please check the flow.");
                    return; 
                }
            }
            ...
            if (node.server.linkStatus !== "connected") {
                RED.log.error("knxUltimateWatchDog: Lost link due to a connection error");
                    ...
                    if (node.listenallga==false) {
                        ...
                        if (msg.destination) {
                           ...
                           // Issue read to all group addresses
                            // 25/10/2019 the user is able not import the csv, so i need to check for it. This option should be unckecked by the knxUltimateWatchDog html config, but..
                    ...
                    setTimeout(() => {
                        if (node.icountMessageInWindow >= 80) {
                            ...
                            setTimeout(() => {
                                ...
                                RED.log.error("knxUltimateWatchDog: Node " + node.id + " has been disabled due to loop detection. Check your flow's design or use RBE option.");
                            }, 1000);
                    ...
                    if (node.server.knxConnection) {
                        ...
                        if (msg.hasOwnProperty('topic')) { 
                            if (msg.topic == grpaddr && msg.knx) {
                                RED.log.error("knxUltimateWatchDog: Circular reference protection. The node " + node.id + " has been disabled. " + JSON.stringify(msg));
                                setTimeout(() => {
