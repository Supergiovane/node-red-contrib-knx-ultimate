      ...
      RED.nodes.registerType('knxUltimateAlerter', {
        ...
        oneditprepare: function () {
          ...
          $('#node-input-rule-container').css('min-height', '350px').css('min-width', '450px').editableList({
            addItem: function (container, i, opt) { // row, index, data
              ...
              const rule = opt.r || { topic: '', devicename: '', longdevicename: '' }
              ...
              const oTopicField = $('<input/>', { class: 'rowRuleTopic', type: 'text', placeholder: 'GA or devicename', style: 'width:20%; margin-left: 5px; text-align: left;' }).appendTo(row)
              const finalspan = $('<span/>', { style: '' }).appendTo(row)
              finalspan.append(' <span class="node-input-rule-index"></span> ')
              const orowRuleDeviceName = $('<input/>', { maxlength: '14', class: 'rowRuleDeviceName', type: 'text', style: 'width:30%; margin-left: 0px; text-align: left;font-style: italic;', placeholder: 'Name (max 14 chars)' }).appendTo(row)
              const orowRuleLongDeviceName = $('<input/>', { class: 'rowRuleLongDeviceName', type: 'text', style: 'width:45%; margin-left: 0px; text-align: left;', placeholder: 'Long name' }).appendTo(row)
        ...
        oneditsave: function () {
          ...
          rules.each(function (i) {
            ...
            const rowRuleLongDeviceName = rule.find('.rowRuleLongDeviceName').val()
    ...
    "loadcontrol": function (RED) {
      ...
      RED.nodes.registerType('knxUltimateLoadControl', {
        ...
        oneditprepare: function () {
          ...
          this.autoComplete = (_Name, _DPT) => {
            const paramAutoComplete = {
              ...
              select: function (event, ui) {
                ...
                const optVal = $("#node-input-dpt option:contains('" + ui.item.label.split('#')[2].trim() + "')").attr('value')
                const $dptSelect = $('#node-input-' + _DPT)
                if (optVal !== undefined && optVal !== null) {
                  $dptSelect.val(optVal).trigger('change')
                } else {
                  $dptSelect.trigger('change')
          ...
          try { const srv = RED.nodes.node($('#node-input-server').val()); if (srv && srv.id) KNX_enableSecureFormatting($('#node-input-topic'), srv.id) } catch (e) {}
          for (let index = 1; index < 6; index++) {
            ...
            try { const srv2 = RED.nodes.node($('#node-input-server').val()); if (srv2 && srv2.id) KNX_enableSecureFormatting($('#node-input-GA' + index), srv2.id) } catch (e) {}
            try { const srv3 = RED.nodes.node($('#node-input-server').val()); if (srv3 && srv3.id) KNX_enableSecureFormatting($('#node-input-MonitorGA' + index), srv3.id) } catch (e) {}
          }
