(function (root, factory) {
  ...
  const MATTER_TYPE_MAP = Object.freeze({
    ...
    // This short-lived beta type has a saved contract compatible with the
  ...
  const inertInstallations = new WeakMap()
  ...
  function open (RED, options = {}) {
    ...
    const offerRecoveryRetry = function () {
      if (recoveryMode && typeof options.onRecoveryRetry === 'function') options.onRecoveryRetry()
    }
    ...
    if (blockedUpgrades.has(RED)) {
      RED.notify(t('upgrade_locked', 'An upgrade is already running or Node-RED must be restarted/reloaded before it can be tried again.'), { type: 'warning', fixed: true })
      return false
    }
    let plan
    try {
    ...
    if (plan.ai.length) {
      const message = makeMessage($, recoveryMode
        ? t('recovery_title', 'Complete KNX Ultimate 8 migration')
        : t('title', 'Upgrade to KNX Ultimate 8'), [
      ...
      RED.notify(message, { type: 'error', fixed: true, modal: true })
      return false
    }
    try {
      validatePreflight(RED, plan)
    ...
    if (RED.user && typeof RED.user.hasPermission === 'function' &&
      ...
      RED.notify(t('permission_denied', 'Your Node-RED account cannot install nodes or deploy flows.'), { type: 'error', fixed: true })
      return false
    }
    ...
    const message = makeMessage($, recoveryMode
      ? t('recovery_title', 'Complete KNX Ultimate 8 migration')
      : t('title', 'Upgrade to KNX Ultimate 8'), recoveryMode
      ? [
          t('recovery_summary', 'Legacy nodes were found after KNX Ultimate 8 was installed. Nodes/configurations to convert: KNX Utility {{utility}}, HUE {{hue}}, Matter {{matter}}.', values),
          t('recovery_confirm', 'A JSON backup of all flows will download first. The required HUE/Matter packages will then be installed and all current editor changes deployed. KNX Ultimate itself will not be reinstalled. IDs, wiring, configuration references, credentials and Matter storage are preserved. The current node editor will close and discard edits not yet saved in this form.', values)
        ]
      : [
    ...
    notification = RED.notify(message, {
      ...
      buttons: [
        ...
        {
          text: recoveryMode
            ? t('recovery_start', 'Back up and complete migration')
            : t('start', 'Back up and upgrade'),
          class: 'primary',
          click: function () {
            if (running) return
            running = true
            ...
            try {
              ...
              RED.notify(recoveryMode
                ? t('recovery_failed', 'Automatic migration stopped safely:') + ' ' + error.message
                : t('failed', 'Automatic upgrade stopped safely. KNX Ultimate 8 was not activated:') + ' ' + error.message, { type: 'error', fixed: true })
              offerRecoveryRetry()
              return
            }
            ...
            Promise.resolve().then(() => runUpgrade(RED, plan, { ...options, environment, $, onProgress }))
              .then(counts => {
                // Unknown config nodes are converted in place. The full Deploy
                // is correct, but a browser reload is required to rebuild their
                // editor indexes and user references from the converted flow.
                keepDeployBlocked = true
                if (progressNotice && typeof progressNotice.close === 'function') progressNotice.close()
                const done = makeMessage($, recoveryMode
                  ? t('recovery_complete_title', 'KNX Ultimate 8 migration completed')
                  : t('complete_title', 'KNX Ultimate 8 installed'), [
                  recoveryMode
                    ? t('recovery_complete', 'Converted {{total}} legacy nodes/configurations. The saved flows are now compatible with KNX Ultimate 8. Reload the editor before making further changes.', counts)
                ...
                doneNotice = RED.notify(done, {
                  ...
                  modal: true,
                  buttons: [{
                    text: recoveryMode ? t('recovery_reload', 'Reload editor') : t('close', 'Close'),
                    click: () => {
                      doneNotice.close()
                      if (recoveryMode && environment.location && typeof environment.location.reload === 'function') environment.location.reload()
                    }
                  }]
                })
              })
              .catch(error => {
                ...
                if (error.code === 'standalone_restart_required') {
                  keepDeployBlocked = true
                  failureMessage = recoveryMode
                    ? t('recovery_standalone_restart_required', 'A HUE/Matter package update is ready. Restart the Node-RED service, reload the editor, then start the migration recovery again. No flows were changed.')
                    : t('standalone_restart_required', 'A HUE/Matter package update is ready. Restart the Node-RED service, reload the editor, then press the upgrade button again. No flows were changed.')
                } else if (error.moduleInstallUncertain) {
                  keepDeployBlocked = true
                  failureMessage = recoveryMode
                    ? t('recovery_module_install_uncertain', 'A package operation may still be running. Restart the Node-RED service, reload the editor and start the migration recovery again. Do not deploy flows before restarting.')
                    : t('module_install_uncertain', 'A package operation may still be running. Restart the Node-RED service, reload the editor and press the upgrade button again. Do not deploy flows before restarting.')
                } else if (error.restartRequired) {
                  keepDeployBlocked = true
                  failureMessage = (recoveryMode
                    ? t('recovery_standalone_restart_required', 'A HUE/Matter package update is ready. Restart the Node-RED service, reload the editor, then start the migration recovery again. No flows were changed.')
                    : t('standalone_restart_required', 'A HUE/Matter package update is ready. Restart the Node-RED service, reload the editor, then press the upgrade button again. No flows were changed.')) + ' ' + error.message
                  ...
                  keepDeployBlocked = true
                  failureMessage = recoveryMode
                    ? t('recovery_flows_persisted', 'The converted flows were saved, but verification did not complete. Do not deploy more changes. Reload the editor and review the saved flows before starting migration recovery again:') + ' ' + error.message
                    : t('flows_persisted', 'The converted flows were saved, but KNX Ultimate 8 was not installed. Do not deploy more changes. Resolve the reported error, reload the editor and press the upgrade button again:') + ' ' + error.message
                } else {
                  failureMessage = recoveryMode
                    ? t('recovery_failed', 'Automatic migration stopped safely:') + ' ' + error.message
  ...
  function install (RED, options = {}) {
    // A running Node-RED 7 process can keep the old plugin HTML in memory while
    // npm has already replaced the package files with version 8. In that state
    // the cached plugin still requests this resource. Do not let it start a
    // second upgrade path: only the plugin shipped and registered by version 8
    // is allowed to opt in to recovery mode.
    if (options.recoveryMode !== true) {
      if (inertInstallations.has(RED)) return inertInstallations.get(RED)
      const inertInstallation = {
        open: function () {},
        dispose: function () { inertInstallations.delete(RED) }
      }
      inertInstallations.set(RED, inertInstallation)
      return inertInstallation
    }
