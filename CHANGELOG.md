...
- **Expose KNX to Matter (BETA) — debounce for staggered Alexa cover commands**: Alexa supports Matter Window Covering, but per Amazon's own developer docs it maps the cluster to its generic `Alexa.RangeController` interface rather than a dedicated cover controller, which can result in a burst of several `GoToLiftPercentage` commands for a single "open/close the blind" voice request instead of one. Forwarding every step to KNX made the physical cover move in jerky stop-start steps. A 2s debounce (same coalescing technique already used for RGB colors) now collapses a burst into a single KNX command using only the last target; `StopMotion` always stays immediate and cancels any pending one.<br/>
