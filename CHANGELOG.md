...
- **Control Matter from KNX — Universal Mode**: the device picker now offers a controller-wide mode that exposes one flow input/output without requiring a KNX gateway or endpoint mappings. One node receives attribute reports, cluster events and initialization notifications from every commissioned Matter device, with complete routing metadata, and accepts dynamic attribute reads/writes and command invocations for any node, endpoint and cluster. Existing endpoint profiles and light behavior remain unchanged.<br/>
