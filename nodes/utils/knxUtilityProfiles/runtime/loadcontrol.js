...
module.exports = function (RED) {
  function knxUltimateLoadControl (config) {
    ...
    const { setTimeout, clearTimeout, setInterval, clearInterval } = profileTimers
    ...
    node.initialread = 1 // Gateway initial-read dispatch expects the numeric mode.
    ...
    if (node.serverKNX) {
      node.serverKNX.removeClient(node)
      if (node.controlMode === 'msg' || node.topic !== '') {
