...
module.exports = function (RED) {
  function knxUltimateAlerter (config) {
    ...
    node.timerSend = null
    const readStatusTimers = new Set()
    ...
    node.on('close', function (done) {
      clearTimeout(node.timerSend)
      readStatusTimers.forEach(timer => clearTimeout(timer))
      readStatusTimers.clear()
      if (node.serverKNX) {
