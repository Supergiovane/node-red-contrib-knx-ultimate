              ...
              try {
                if (node.exposedGAs.length > 0) {
                  ...
                  if (oExposedGA !== undefined) {
                    ...
                    const msg = buildInputMessage({ _srcGA: '', _destGA: oClient.topic, _event: 'GroupValue_Response', _Rawvalue: Buffer.from(oExposedGA.rawValue.data), _inputDpt: oClient.dpt, _devicename: oClient.name ? oClient.name : '', _outputtopic: oClient.outputtopic, _oNode: oClient })
          ...
          node.nodeClients
            ...
            .forEach(_input => {
              ...
              if (input.hasOwnProperty('isSceneController')) {
                ...
                if (_dest === input.topic || _dest === input.topicSave) {
                  ...
                  new Promise((resolve, reject) => {
                    ...
                    if (_dest === input.topicSave) {
                      try {
                        const msgSave = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: _rawValue, _inputDpt: input.dptSave, _devicename: input.name ? input.name : '', _outputtopic: _dest, _oNode: null })
                  ...
                  new Promise((resolve, reject) => {
                    ...
                    for (let i = 0; i < input.rules.length; i++) {
                      ...
                      if (typeof oDevice !== 'undefined' && oDevice.topic == _dest) {
                        const msg = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: _rawValue, _inputDpt: oDevice.dpt, _devicename: oDevice.name ? input.name : '', _outputtopic: oDevice.outputtopic, _oNode: null })
                ...
                const msg = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: _rawValue, _inputDpt: (typeof oGA === 'undefined') ? null : oGA.dpt, _devicename: (typeof oGA === 'undefined') ? input.name || '' : oGA.devicename, _outputtopic: _dest, _oNode: input })
                ...
                if (input.hasOwnProperty('isWatchDog')) { // 04/02/2020 Watchdog implementation
                  ...
                  const msg = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: _rawValue, _inputDpt: input.dpt, _devicename: input.name ? input.name : '', _outputtopic: input.outputtopic, _oNode: input })
          ...
          node.nodeClients
            ...
            .forEach(_input => {
              ...
              if (input.hasOwnProperty('isLogger')) { // 26/03/2020 Coronavirus is slightly decreasing the affected numer of people. Logger Node
                ...
                // Read Request
                const msg = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: null, _inputDpt: (typeof oGA === 'undefined') ? null : oGA.dpt, _devicename: (typeof oGA === 'undefined') ? input.name || '' : oGA.devicename, _outputtopic: _dest, _oNode: input })
                ...
                if (input.hasOwnProperty('isWatchDog')) {
                  ...
                  // Read Request
                  const msg = buildInputMessage({ _srcGA: _src, _destGA: _dest, _event: _evt, _Rawvalue: null, _inputDpt: input.dpt, _devicename: input.name ? input.name : '', _outputtopic: input.outputtopic, _oNode: input })
