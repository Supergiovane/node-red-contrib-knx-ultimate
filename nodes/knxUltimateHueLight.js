      ...
      try {
        switch (msg.knx.destination) {
          case config.GALightSwitch:
            ...
            node.setNodeStatusHue({
              fill: 'green', shape: 'dot', text: 'KNX->HUE', payload: state,
            });
            break;
          case config.GALightDIM:
            ...
            node.setNodeStatusHue({
              fill: 'green', shape: 'dot', text: 'KNX->HUE', payload: msg.payload,
            });
            break;
          case config.GADaylightSensor:
            if (config.enableDayNightLighting === true) {
              ...
              node.setNodeStatusHue({
                fill: 'green', shape: 'dot', text: 'KNX->HUE Daytime', payload: node.DayTime,
              });
            } else {
              node.DayTime = true;
            }
          ...
          case config.GALightBlink:
            ...
            node.setNodeStatusHue({
              fill: 'green', shape: 'dot', text: 'KNX->HUE', payload: gaVal,
            });
            break;
          case config.GALightColorCycle:
            ...
            node.setNodeStatusHue({
              fill: 'green', shape: 'dot', text: 'KNX->HUE', payload: gaValColorCycle,
            });
            break;
          default:
            break;
        ...
        (async () => {
          try {
            ...
            await node.serverHue.hueManager.writeHueQueueAdd(config.hueDevice, null, (node.isGrouped_light === false ? 'getLight' : 'getGroupedLight'), (jLight) => {
              ...
              node.setNodeStatusHue({
                fill: 'blue', shape: 'ring', text: 'Connected. Is on: ', payload: node.currentHUEDevice.on.on === true,
              });
            });
          } catch (err) {
