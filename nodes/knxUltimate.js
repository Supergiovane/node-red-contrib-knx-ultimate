  ...
  function knxUltimate(config) {
        ...
        if (node.server.knxConnection) {
          ...
          if (outputtype == 'response') {
            try {
              ...
              node.server.writeQueueAdd({ grpaddr, payload: msg.payload, dpt, outputtype, nodecallerid: node.id })
              node.setNodeStatus({ fill: 'blue', shape: 'dot', text: 'Responding', payload: msg.payload, GA: grpaddr, dpt, devicename: '' })
            } catch (error) { }
            ...
            try {
              ...
              node.server.writeQueueAdd({ grpaddr, payload: msg.payload, dpt, outputtype, nodecallerid: node.id })
              node.setNodeStatus({ fill: 'grey', shape: 'dot', text: 'Updating internal value', payload: msg.payload, GA: grpaddr, dpt, devicename: '' })
            } catch (error) { }
          } else {
            try {
              node.currentPayload = msg.payload// 31/12/2019 Set the current value (because, if the node is a virtual device, then it'll never fire "GroupValue_Write" in the server node, causing the currentPayload to never update)
              ...
              node.server.writeQueueAdd({ grpaddr, payload: msg.payload, dpt, outputtype, nodecallerid: node.id })
              node.setNodeStatus({ fill: 'green', shape: 'dot', text: 'Writing', payload: msg.payload, GA: grpaddr, dpt, devicename: '' })
              // } else {
    ...
    if (node.server) {
      ...
      if (node.server.linkStatus === 'connected' && node.initialread === 1 || node.initialread === 3) {
        ...
        const t = setTimeout(function () { // 21/03/2022 fixed possible memory leak. Previously was setTimeout without "let t = ".
